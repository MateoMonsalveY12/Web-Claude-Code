...
export default function Navbar() {
  ...
  const runSearch = useCallback(async (q) => {
    if (!supabase || q.length < 2) { setSearchResults([]); setSearching(false); return }
    setSearching(true)
    try {
  ...
  return (
    <>
      ...
      >
        ...
        <div className="container-brand flex items-center justify-between h-14 md:h-[72px]">
          ...
          <div className="flex items-center gap-1">
            <button onClick={openSearch} className={`nav-icon-btn transition-colors duration-300 ${textColor}`} aria-label="Buscar">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round">
                <circle cx="11" cy="11" r="7"/><line x1="21" y1="21" x2="16.65" y2="16.65"/>
              </svg>
            </button>

            {/* User auth button — desktop only */}
            <div className="hidden md:block relative" ref={userMenuRef}>
              {user ? (
                <>
                  <button
                    onClick={() => setUserMenuOpen(v => !v)}
                    className={`nav-icon-btn flex items-center gap-1.5 transition-colors duration-300 ${textColor}`}
                    aria-label="Mi cuenta"
                  >
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round">
                      <path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/>
                    </svg>
                    <span className="font-sans text-xs font-semibold max-w-[80px] truncate">
                      {user.email?.split('@')[0]}
                    </span>
                  </button>
                  {userMenuOpen && (
                    <div className="absolute right-0 top-full mt-1 w-44 bg-white border border-brand-border shadow-md z-50">
                      <Link
                        to="/mi-cuenta/pedidos"
                        className="block px-4 py-3 font-sans text-sm text-brand-black hover:bg-brand-gray transition-colors"
                      ...
                      <button
                        ...
                        className="w-full text-left px-4 py-3 font-sans text-sm text-brand-black/60 hover:bg-brand-gray transition-colors border-t border-brand-border"
                      >
                        Cerrar sesión
                      </button>
                    </div>
      ...
      {searchOpen && (
        ...
        >
          {/* Backdrop */}
          <div className="absolute inset-0 bg-black/50" onClick={closeSearch} aria-hidden="true" />
          {/* Panel */}
          <div className="relative bg-white w-full shadow-xl">
            <div className="container-brand py-4 flex items-center gap-3">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" className="text-brand-black/40 flex-shrink-0">
                <circle cx="11" cy="11" r="7"/><line x1="21" y1="21" x2="16.65" y2="16.65"/>
              </svg>
              <input
                ref={searchInputRef}
                ...
                className="flex-1 font-sans text-sm text-brand-black placeholder-brand-black/30 outline-none bg-transparent"
                autoComplete="off"
              />
              {searching && (
                <div className="w-4 h-4 border-2 border-brand-black/20 border-t-brand-black/60 rounded-full animate-spin flex-shrink-0" />
              )}
              <button onClick={closeSearch} aria-label="Cerrar búsqueda" className="flex-shrink-0 text-brand-black/40 hover:text-brand-black transition-colors">
                ...
                <div className="container-brand py-3">
                  {searchResults.map(p => (
                    <Link
                      ...
                      to={`/products/${p.slug}`}
                      onClick={closeSearch}
                      className="flex items-center gap-4 py-3 hover:bg-brand-gray -mx-4 px-4 transition-colors"
                    >
                      <div className="w-12 h-12 flex-shrink-0 bg-brand-gray overflow-hidden">
                        <img
                          src={p.images?.[0] || '/images/product-1.jpg'}
                          alt={p.name}
                          className="w-full h-full object-cover object-top"
                        />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="font-sans text-xs uppercase tracking-button font-semibold text-brand-black truncate">{p.name}</p>
                        <p className="font-sans text-xs text-brand-black/50 mt-0.5">{'$ ' + p.price.toLocaleString('es-CO')}</p>
                      </div>
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" className="text-brand-black/30 flex-shrink-0">
            ...
            {searchQuery.length >= 2 && !searching && searchResults.length === 0 && (
              <div className="border-t border-brand-border">
                <div className="container-brand py-5 text-center">
                  <p className="font-sans text-sm text-brand-black/50">No encontramos prendas para "{searchQuery}"</p>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="px-5 pb-8 pt-4 border-t border-brand-border space-y-3">
          {user ? (
            <div className="flex items-center justify-between py-2 border-b border-brand-border">
              <span className="font-sans text-sm text-brand-black/70">{user.email?.split('@')[0]}</span>
              <button
                onClick={() => { signOut(); setMobileOpen(false) }}
                className="font-sans text-xs underline text-brand-black/50"
              >
                Cerrar sesión
              </button>
            </div>
          ...
          <a href="https://wa.me/573102232188" className="flex items-center gap-2 font-sans text-sm text-brand-black">
